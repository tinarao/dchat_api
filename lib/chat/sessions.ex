defmodule Chat.Sessions do
  ...
  def create_session(session_key, user_name, device_id) do
    ...
    Chat.Redis.set_session(session_key, session_data, @session_ttl)
  end

  def get_session(session_key) do
    case Chat.Redis.get_session(session_key) do
  ...
  def session_exists?(session_key) do
    Chat.Redis.session_exists?(session_key)
  end
  ...
  def update_activity(session_key) do
    case get_session(session_key) do
      {:ok, session_data} ->
        ...
        Chat.Redis.set_session(session_key, updated_data, @session_ttl)
  ...
  def delete_session(session_key) do
    Chat.Redis.delete_session(session_key)
  end
